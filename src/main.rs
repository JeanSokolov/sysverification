...
fn main() -> std::io::Result<()> {
    let gcs_simulation_cycles: i32 = std::env::args().nth(2).unwrap().parse().unwrap();
    let bcs_simulation_cycles: i32 = std::env::args().nth(3).unwrap().parse().unwrap();

    println!(
        "Performing {} cycles with {} stuck ats each",
        gcs_simulation_cycles, bcs_simulation_cycles
    );
    ...
    for l in 0..gcs_simulation_cycles {
        ...
        for (_, gate) in network.iter_mut() {
            ...
            if gate.input.iter().any(|item| item.is_none()) {
                ...
                gate.value = Some(rdm);
                data[&l.to_string()]["test_pattern"][&gate.name] = (rdm as i32).into();
            }
        }
        ...
        for i in gates.clone() {
            ...
            data[&l.to_string()]["gcs"][&gate.name] = (gate.value.unwrap() as i32).into();
        }

        for n in 0..bcs_simulation_cycles {
                ...
                if unlocked_gate.kind.eq(&1) {
                    if rand::thread_rng().gen::<bool>() {
                        if !stuck_ats.contains(&unlocked_gate.name) {
                            ...
                            data[&l.to_string()][&format!("bcs_iteration_{}", n)]["stuck_at"] =
                                (format!(
                                    "input[0]={} for Gate: {}",
                                    rdm as i32, unlocked_gate.name
                                ))
            ...
            if gcs_values.ne(&bcs_values) {
                for i in 0..gcs_names.clone().len() {
                    ...
                    data[&l.to_string()][&format!("bcs_iteration_{}", n)][&gcs_names[i]] =
                        (bcs_values[i].unwrap() as i32).into();
                }
            } else {
                data[&l.to_string()][&format!("bcs_iteration_{}", n)]["result"] =
                    ("same as gcs").into();
